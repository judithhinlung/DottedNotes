...
from dottednotes.models.ornament import Ornament, OrnamentType
    ...
    def _parse_music_tokens(self, tokens: list[str], staff_name: Optional[str] = None) -> Staff:
        ...
        while i < len(tokens):
            ...
            # Check if note or rest
                ...
                for idx, pitch_tok in enumerate(chord_pitches):
                ...
                if pitch_info:
                    ...
                    current_measure.add_note(n_obj)
                else:
                    i += 1
    ...
    def _parse_note_decorations(self, tokens: list[str], start_idx: int, note: Note) -> int:
        ...
        while i < len(tokens):
            ...
            if t == '~':
                ...
                note.dynamics.append(Dynamic(dots=frozenset(), category=None, raw_brl="", level=_DYNAMIC_MAP[t[1:]]))
                i += 1
